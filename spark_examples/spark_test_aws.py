...
print('reading csv')
train = spark.read.csv("s3://daveandhuey/train_numeric.csv", header="true", inferSchema="true")
...
# Train a GBT model.
gbt = GBTClassifier(labelCol="Response", featuresCol="features", maxIter=1)
...
data_test = spark.read.csv("s3://daveandhuey/test_numeric.csv", header="true", inferSchema="true")
...
sub = pd.read_csv("sample_submission.csv")  # Pandas dependency on boto makes using s3 not work
